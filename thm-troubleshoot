...
EOF
if [[ -n $distro ]]; then
  printf "\033[38;2;216;1;81mLooks like you're running %s \033[0m" "$distro"
else
  printf "\033[38;2;216;1;81mLinux distro not recognized \033[0m"
fi
...
# Determine package manager being used to create variables
if pacman -V &>/dev/null; then 
  pkg_manager_status=("pacman" "-V");
  is_openvpn_installed=("pacman" "-Qs" "openvpn");
  pkg_manager_update=("pacman" "-Syy");
  install_openvpn=("pacman" "-S" "openvpn" "--noconfirm");
fi

if dpkg-query -W -f='${Status}' apt &>/dev/null; then
  pkg_manager_status=("dpkg-query" "-W" "-f='${Status}'" "apt")
  is_openvpn_installed=("dpkg-query" "-W" "-f='${Status}'" "openvpn")
  pkg_manager_update=("apt" "update")
  install_openvpn=("apt" "install" "openvpn" "-y")
fi

#Ensure that Openvpn is installed
if ! "${is_openvpn_installed[@]}" &>/dev/null; then
	...
	case "$choice" in
		n|N)
			...
			;;
		*)
			if  ! "${pkg_manager_status[@]}" &>/dev/null; then
				colour red "[-] System doesn't use apt or pacman -- please install OpenVPN manually" 1
				colour red "[-] Exiting" 2
				fin
			else
				"${pkg_manager_update[@]}" &>/dev/null && "${install_openvpn[@]}" &>/dev/null & pid=$!
